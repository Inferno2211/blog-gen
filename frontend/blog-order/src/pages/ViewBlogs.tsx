...
const ViewBlogs = () => {
  ...
  const domains = [
    ...new Set(articles.map((a) => a.domain?.slug).filter(Boolean)),
  ];
  ...
  const handleSave = async (id: string) => {
    try {
      ...
      setArticles(
        articles.map((a) => (a.id === id ? { ...a, ...editForm } : a))
      );
  ...
  const handlePublish = async (articleId: string) => {
    try {
      ...
      alert(
        "Failed to publish article. Please check if the article has a domain and selected version."
      );
    } finally {
      setPublishing(null);
    }
  ...
  const filteredAndSortedArticles = articles
    .filter((article) => {
      if (filters.domain && article.domain?.slug !== filters.domain)
        return false;
    ...
    <div className="max-w-7xl mx-auto py-10">
      <div className="mb-8">
        ...
        <p className="text-gray-600 dark:text-gray-400">
          Manage all articles, versions, and publishing
        </p>
      </div>
                    ...
                    <div className="text-sm">
                      ...
                      {article.versions && article.versions.length > 0 && (
                        <select
                          ...
                          onChange={(e) =>
                            handleVersionChange(article.id, e.target.value)
                          }
                        ...
                        >
                          ...
                          {article.versions.map((version: ArticleVersion) => (
                            <option key={version.id} value={version.id}>
                              ...
                              {version.id === article.selected_version_id
                                ? "(Selected)"
                                : ""}
                            </option>
                          ))}
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditForm({
                      ...editForm,
                      backlink_target: e.target.value,
                    })
                  }
                  className="w-full p-2 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                />
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditForm({ ...editForm, anchor: e.target.value })
                  }
                  className="w-full p-2 border border-gray-300 dark:border-gray-600 rounded bg-white dark:bg-gray-700 text-gray-900 dark:text-gray-100"
                />
              </div>
