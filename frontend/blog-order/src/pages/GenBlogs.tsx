...
const GenBlogs = () => {
  ...
  const handleChange = (
    ...
    setForm((prev) => ({
      ...prev,
      [name]:
        type === "checkbox" ? checked : name === "n" ? Number(value) : value,
    }));
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setError("");
    try {
      const res = await generateBlog(form);
      setBlog(res);
  ...
  const handleNewVersion = async () => {
    ...
    try {
      const res = await generateBlogVersion({
        ...
        internalLinkEnabled: form.internalLinkEnabled,
        noExternalBacklinks: form.noExternalBacklinks,
      });
  ...
  return (
    <div className="max-w-4xl mx-auto py-10">
      <h1 className="text-3xl font-bold mb-8 text-center text-gray-900 dark:text-gray-100">
        Buy a Blog - Private Blog Network
      </h1>
      ...
      <div className="mb-6 flex flex-col sm:flex-row items-center justify-center gap-4">
        <label className="flex items-center cursor-pointer">
          <input
            type="checkbox"
            name="internalLinkEnabled"
            checked={form.internalLinkEnabled}
            onChange={handleChange}
            className="sr-only"
          />
          <div
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
              form.internalLinkEnabled
                ? "bg-green-600"
          ...
          >
            <span
              className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                form.internalLinkEnabled ? "translate-x-6" : "translate-x-1"
              }`}
            />
          </div>
          <span className="ml-3 text-sm font-medium text-gray-900 dark:text-gray-100">
        ...
        <label className="flex items-center cursor-pointer">
          <input
            type="checkbox"
            name="noExternalBacklinks"
            checked={form.noExternalBacklinks}
            onChange={handleChange}
            className="sr-only"
          />
          <div
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
              form.noExternalBacklinks
                ? "bg-red-600"
                : "bg-gray-300 dark:bg-gray-600"
            }`}
          >
            <span
              className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                form.noExternalBacklinks ? "translate-x-6" : "translate-x-1"
              }`}
            />
          </div>
          <span className="ml-3 text-sm font-medium text-gray-900 dark:text-gray-100">
      ...
      <form
        className={`grid grid-cols-1 md:grid-cols-2 gap-6 bg-white/5 p-8 rounded-xl shadow transition-all duration-300 ${
          blog
            ? "opacity-60 pointer-events-none h-0 overflow-hidden p-0 m-0"
            : ""
        }`}
      ...
      {blog && (
        <div className="flex flex-col md:flex-row gap-4 justify-center mt-8">
          ...
          <button
            ...
            onClick={() =>
              selectedVersion && handleSelectVersion(selectedVersion)
            }
        ...
        <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
          <div className="bg-white dark:bg-gray-800 p-8 rounded-xl shadow-xl max-w-sm w-full">
            ...
            <p className="mb-6 text-gray-700 dark:text-gray-300">
              This will clear all drafts and versions. Are you sure?
            </p>
        ...
        <div className="mt-10">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {drafts.map((v, idx) => (
              <div
                key={v.versionId || idx}
                className={`rounded-xl shadow p-4 cursor-pointer border-2 transition-all ${
              ...
              >
                <div className="font-bold text-lg mb-2 text-gray-900 dark:text-gray-100">
                  {parseMarkdownWithFrontmatter(v.content).frontmatter.title ||
                    `Version ${v.versionNum ?? idx + 1}`}
                </div>
